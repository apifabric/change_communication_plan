...
resourcename="changecommunicationplan"  # lower case, only
resourcegroup="changecommunicationplan_rg"
...
# git clone https://github.com/apilogicserver/changecommunicationplan.git
# cd changecommunicationplan
...
az group create --name changecommunicationplan_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group changecommunicationplan_rg --sku S1 --is-linux
az container create --resource-group changecommunicationplan_rg --name changecommunicationplan --image apilogicserver/changecommunicationplan:latest --dns-name-label changecommunicationplan --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//changecommunicationplan.westus.azurecontainer.io:5656'
